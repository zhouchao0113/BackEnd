...
## [C语言是怎么进行函数调用的？](https://blog.csdn.net/yanerhao/article/details/69258885)
...
* 大端
  * 若16进制的**高字节存放在内存的低地址**，则为大端字节序，34存储在高位，12存储在低位
* 小端
  * 若16进制的**低字节存放在内存的低地址**，则为小端字节序，12存储在高位，34存储在高位

### 代码判断

在**union中所有的数据成员共用一个空间**，**同一时间只能储存其中一个数据成员**，所有的数据成员具有相同的起始地址。即下述的union虽然定义了两个成员，但其实这个union只占用了4个字节(32位机器中)，**往a成员赋值，然后读取b就相当于读取a成员的低位第一个字节的值**。如果机器使用大端模式，则u.a=1那a的最高字节值为1；如果机器使用小端模式，则u.a=1则a的最低位字节为1。上述可知b和a有相同的起始位，所以读取b如果等于1，则为小端模式，b为0则为大端模式
